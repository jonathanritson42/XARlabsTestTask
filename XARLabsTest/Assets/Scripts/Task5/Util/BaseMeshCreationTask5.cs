...
public class BaseMeshCreationTask5 : MonoBehaviour
{

    [Header("Lissajous Animation")]

    [SerializeField] protected bool useAnimation;
    [SerializeField] protected bool useRandom;

    [Space]

    [Tooltip("The minimum random value for the animation")]
    [SerializeField] protected float minimumRandomLissajousValue;

    [Tooltip("The maximum random value for the animation")]
    [SerializeField] protected float maximumRandomLissajousValue;
    ...
    [HideInInspector]
    public LissajousAnimation lissajousAnimation;
    ...
    public void RandomLissajouseAnimation(LissajousAnimation animation) {

        for (int i = 0; i < Enum.GetValues(typeof(LissajousValues)).Length; i++)
        {
            animation.RandomiseLissajousValue((LissajousValues)i, minimumRandomLissajousValue, maximumRandomLissajousValue);
        }
    }

#if UNITY_EDITOR

    [ContextMenu("Randomise Animation Values")]
    public void RandomiseAnimationValues()
    {
        if (lissajousAnimation == null) return;

        for (int i = 0; i < Enum.GetValues(typeof(LissajousValues)).Length; i++)
        {
            lissajousAnimation.RandomiseLissajousValue((LissajousValues)i, minimumRandomLissajousValue, maximumRandomLissajousValue);
        }
    }
#endif
}